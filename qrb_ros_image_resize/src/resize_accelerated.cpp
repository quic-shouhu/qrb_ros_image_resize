...
void ResizeNode::handle_callback(
  ...
  int status = utils_.image_resize(image_msg.dmabuf->fd(), image_msg.width,
                                   image_msg.height, out_fd, out_msg->width,
                                   out_msg->height, encoding, interpolation_);
  if (calculate_enable_) {
  ...
  if (status != 0) {
    RCLCPP_ERROR(this->get_logger(),
                 "Resize failed with status: %d, output fd: %d, encoding: %s",
