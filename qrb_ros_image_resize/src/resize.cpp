...
void ResizeNode::handle_callback(const qrb_ros::transport::type::Image & image_msg)
{
  ...
  read_image_from_dmabuf(image_msg.dmabuf, data.get(), image_msg.width, image_msg.height, step,
      image_msg.encoding, true);
  ...
  if (use_scale_) {
    ...
    cv::resize(
        rgb->image, resized_image, cv::Size(out_msg->width, out_msg->height), 0, 0, interp_flag);
  }
  if (calculate_enable_) {
  ...
  cv::Mat v_plane_d = yuv_image(cv::Rect(uv_width, out_msg->height, uv_width, uv_height));
  // Initialize NV12 data array
  std::vector<uchar> nv12_data(out_msg->width * out_msg->height * 3 / 2);
  // Copy Y plane
  for (int32_t i = 0; i < out_msg->height; ++i) {
    memcpy(
        nv12_data.data() + i * out_msg->width, y_plane_d.data + i * out_msg->width, out_msg->width);
  }
  // Copy UV plane
  ...
  step = out_msg->width;
  if (!save_image_to_dmabuf(
          dmabuf, nv12_data.data(), out_msg->width, out_msg->height, step, encoding, true)) {
