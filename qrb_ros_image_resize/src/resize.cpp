...
ResizeNode::ResizeNode(const rclcpp::NodeOptions &options)
  ...
  image_sub_ = this->create_subscription<qrb_ros::transport::type::Image>(
      "image_raw", 10,
      std::bind(&ResizeNode::handle_callback, this, std::placeholders::_1),
      sub_options);

  rclcpp::PublisherOptions pub_options;
  pub_options.use_intra_process_comm = rclcpp::IntraProcessSetting::Enable;
  image_pub_ = this->create_publisher<qrb_ros::transport::type::Image>(
      "image_resize", 10, pub_options);
...
void ResizeNode::handle_callback(
    const qrb_ros::transport::type::Image &image_msg) {
  ...
  if (use_scale_) {
    if (scale_height_ < LIMIT_SCALE || scale_width_ < LIMIT_SCALE ||
      ...
      RCLCPP_ERROR(this->get_logger(),
                   "The dst image scale_width/height is not supported");
      return;
    }
  } else {
    if (out_msg->height * 8 < image_msg.height ||
        out_msg->width * 8 < image_msg.width) {
      RCLCPP_ERROR(this->get_logger(),
                   "The dst image width/height scale is not supported");
      return;
    }
  }
  ...
  if (encoding != "nv12") {
    RCLCPP_ERROR(this->get_logger(), "Image encoding is not supported: %s",
                 encoding.c_str());
    return;
  }
  ...
  if (interpolation_ != 0 && interpolation_ != 1) {
    RCLCPP_ERROR(this->get_logger(), "Image interpolation is not supported: %d",
                 interpolation_);
    return;
  }
  cv::InterpolationFlags interp_flag =
      static_cast<cv::InterpolationFlags>(interpolation_ + 2);
  ...
  read_image_from_dmabuf(image_msg.dmabuf, data.get(), image_msg.width,
                         image_msg.height, step, image_msg.encoding, true);
  ...
  cv::cvtColorTwoPlane(y_plane_s, uv_plane_s, rgb->image,
  ...
  if (use_scale_) {
    cv::resize(rgb->image, resized_image, cv::Size(), scale_width_,
               scale_height_, interp_flag);
  } else {
  ...
  cv::Mat y_plane_d =
      yuv_image(cv::Rect(0, 0, out_msg->width, out_msg->height));
  cv::Mat u_plane_d =
      yuv_image(cv::Rect(0, out_msg->height, uv_width, uv_height));
  cv::Mat v_plane_d =
      yuv_image(cv::Rect(uv_width, out_msg->height, uv_width, uv_height));
  ...
  for (uint32_t j = 0; j < uv_height; j++) {
    for (uint32_t i = 0; i < uv_width; i++) {
      nv12_data[nLenY + j * out_msg->width + 2 * i] =
          u_plane_d.data[j * uv_width + i];
      nv12_data[nLenY + j * out_msg->width + 2 * i + 1] =
          v_plane_d.data[j * uv_width + i];
    }
  }

  // Allocate dmabuf
  auto dst_size =
      get_image_align_size(out_msg->width, out_msg->height, encoding);
  auto dmabuf =
      lib_mem_dmabuf::DmaBuffer::alloc(dst_size, "/dev/dma_heap/system");
  if (dmabuf == nullptr) {
    RCLCPP_ERROR(rclcpp::get_logger("image_test"), "dma buffer alloc failed");
  }
  step = out_msg->width;
  if (!save_image_to_dmabuf(dmabuf, nv12_data.data(), out_msg->width,
                            out_msg->height, step, encoding, true)) {
  ...
  if (calculate_enable_) {
    ...
    auto time_resize = std::chrono::duration_cast<std::chrono::microseconds>(
                           time_resize_end_ - time_resize_start_)
                           .count();
    auto time_node = std::chrono::duration_cast<std::chrono::microseconds>(
                         time_node_end_ - time_node_start_)
                         .count();
...
void ResizeNode::calculate_fps_and_latency() {
  ...
  double fps =
      static_cast<double>(frame_cnt_) / static_cast<double>(CALCULATE_TIME);
  if (total_latency_ <= static_cast<double>(CALCULATE_TIME) * 1000000.0 &&
      total_latency_ > 0) {
